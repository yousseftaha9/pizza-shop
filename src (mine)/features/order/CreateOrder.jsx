...
import { getTotalCartPrice, makeOrderPriority } from '../cart/cartSlice';
...
function CreateOrder() {
  ...
  const totalWithNoPriority = useSelector(getTotalCartPrice);

  let totalWithPriority = useSelector(makeOrderPriority);
  totalWithPriority = totalWithPriority + 0.2 * totalWithPriority;

  function changePriority() {
    setWithPriority((priority) => !priority);
  }
        ...
        <div>
          ...
          <Button disabled={isSubmitting} type="primary">
            {isSubmitting
              ? 'Placing order...'
              : withPriority
                ? `Order Now From $${totalWithPriority}`
                : `Order Now From $${totalWithNoPriority}`}
          </Button>
        </div>
      </Form>
    </div>
...
//without writing all the boilerplate code
